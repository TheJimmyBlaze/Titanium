...
export const useButton = ({
    ...
    const setHover = hover => state.hover = state.hover || hover;
    ...
    const update = () => {
        ...
        if (collider.overlaps(mouseCollider).length) {
    ...
    const draw = () => {
        ...
        drawCamera.requestDraw(ctx => {
            ...
            ctx.textAlign = 'right';
            if (!state.hover) {
                ctx.strokeStyle = ctx.fillStyle = !state.disabled && style[getBackground()] || getColour();
            }
