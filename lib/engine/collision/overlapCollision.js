...
export const colliderOverlaps = (a, b) => {
    ...
    if (aType === collisionTypes.line) {
        if (bType === collisionTypes.rect) return lineRectOverlap(a, b);
        if (bType === collisionTypes.circle) return lineCircleOverlap(a, b);
        return lineOverlap(a, b);
    }

    if (aType === collisionTypes.circle) {
        if (bType === collisionTypes.rect) return circleRectOverlap(a, b);
        if (bType === collisionTypes.line) return lineCircleOverlap(b, a);
        return circleOverlap(a, b);
    }

    if (aType === collisionTypes.rect) {
        if (bType === collisionTypes.circle) return circleRectOverlap(b, a);
        if (bType === collisionTypes.line) return lineRectOverlap(b, a);
