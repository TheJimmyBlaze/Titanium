...
const lineLineResolve = (a, b) => {
    ...
    const { x: aX2, y: aY2 } = aEnd.getPosition();
    ...
    if (determinate === 0) {
        return b.line.findClosestPositionOnLine(aEnd);
    }
...
const lineRectResolve = (a, b) => {
    ...
    if (b.contains(usePointCollider({ position: aStart })) || b.contains(usePointCollider({ position: aEnd }))) {
        const { x: ex, y: ey } = aEnd.getPosition();
        const point = closestPointOnRectEdge(bX, bY, bWidth, bHeight, ex, ey);
        return usePosition(point);
    }

    const { x: endX, y: endY } = aEnd.getPosition();
    const rectPoint = () => closestPointOnRectEdge(bX, bY, bWidth, bHeight, endX, endY);
...
const lineCircleResolve = (a, b) => {
    ...
    const { x: bX, y: bY } = b.position.getPosition();
    const { x: endX, y: endY } = a.line.getEndPosition().getPosition();
    const point = closestPointOnCircleEdge(bX, bY, bRadius, endX, endY);
    return usePosition(point);
};
